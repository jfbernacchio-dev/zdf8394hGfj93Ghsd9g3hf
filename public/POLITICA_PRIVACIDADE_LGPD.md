# POLÍTICA DE PRIVACIDADE E PROTEÇÃO DE DADOS PESSOAIS

**Última atualização:** [DATA]

---

## 1. IDENTIFICAÇÃO DO CONTROLADOR DE DADOS

Esta Política de Privacidade se aplica aos serviços prestados por:

**Nome/Razão Social:** [NOME DO PSICÓLOGO OU CLÍNICA]  
**CPF/CNPJ:** [NÚMERO]  
**CRP:** [NÚMERO DE REGISTRO NO CONSELHO]  
**Endereço:** [ENDEREÇO COMPLETO]  
**Telefone:** [TELEFONE]  
**E-mail:** [E-MAIL]  
**Encarregado de Dados (DPO):** [NOME DO RESPONSÁVEL OU PRÓPRIO PSICÓLOGO]

---

## 2. INTRODUÇÃO E COMPROMISSO COM A PRIVACIDADE

Este documento tem como objetivo esclarecer como são coletados, utilizados, armazenados e protegidos os dados pessoais dos pacientes, em conformidade com:

- **Lei Geral de Proteção de Dados (LGPD)** – Lei nº 13.709/2018;
- **Código de Ética Profissional do Psicólogo** – Resolução CFP nº 10/2005;
- **Resolução CFP nº 01/2009** – Sobre a manutenção de documentação;
- **Constituição Federal** – Direito à privacidade e sigilo profissional.

Nosso compromisso é garantir a **confidencialidade, integridade e segurança** de todas as informações coletadas durante o processo terapêutico.

---

## 3. DADOS PESSOAIS COLETADOS

### 3.1. Dados Cadastrais
- Nome completo
- CPF
- Data de nascimento
- Endereço residencial
- Telefone e e-mail
- Informações de contato de emergência (se aplicável)

### 3.2. Dados Sensíveis (Saúde)
Conforme a **LGPD (art. 5º, II e art. 11)**, dados relacionados à saúde são considerados **sensíveis** e recebem tratamento especial, incluindo:
- Histórico clínico e psicológico
- Registros de sessões de psicoterapia (prontuário)
- Diagnósticos, avaliações psicológicas e evoluções terapêuticas
- Prescrições e encaminhamentos médicos (quando aplicável)
- Informações sobre familiares, quando relevantes ao tratamento

### 3.3. Dados Financeiros
- Informações sobre pagamentos de sessões
- Dados fiscais para emissão de notas fiscais eletrônicas (NFSe)
- Histórico de fechamentos e faturas

### 3.4. Documentos e Arquivos
- Documentos complementares ao tratamento (laudos médicos, exames, relatórios)
- Arquivos anexados pelo profissional durante o acompanhamento terapêutico
- Imagens ou documentos relevantes ao processo terapêutico

### 3.5. Dados de Sistema e Auditoria
- Logs de acesso administrativo para fins de segurança e auditoria
- Registros de autenticação e controle de acesso
- Histórico de alterações em dados críticos

---

## 4. FINALIDADES DO TRATAMENTO DE DADOS

Os dados pessoais coletados são utilizados **exclusivamente** para as seguintes finalidades:

✅ **Prestação de serviços de psicoterapia**, incluindo avaliação, acompanhamento e evolução do tratamento;  
✅ **Elaboração e manutenção de prontuário clínico**, conforme exigido pela Resolução CFP nº 01/2009;  
✅ **Agendamento e controle de sessões**, com envio de lembretes e confirmações através de notificações;  
✅ **Gestão administrativa e financeira**, incluindo controle de pagamentos e emissão de notas fiscais eletrônicas (NFSe);  
✅ **Armazenamento seguro de documentos** relacionados ao tratamento psicológico;  
✅ **Comunicação com o paciente** sobre assuntos relacionados ao tratamento;  
✅ **Controle de acesso e auditoria** para garantia de segurança das informações;  
✅ **Cumprimento de obrigações legais**, como atendimento a requisições judiciais, do Conselho Federal de Psicologia (CFP) ou de outras autoridades competentes.

**Importante:** Nenhum dado será utilizado para fins comerciais, publicitários ou compartilhado com terceiros sem o consentimento expresso do paciente, exceto nas hipóteses legais previstas.

---

## 5. BASE LEGAL PARA O TRATAMENTO DE DADOS

O tratamento de dados pessoais é realizado com base nas seguintes hipóteses legais previstas na **LGPD (art. 7º e 11)**:

| Base Legal | Aplicação |
|------------|----------|
| **Consentimento do titular** | Coleta de dados cadastrais e de saúde para fins terapêuticos |
| **Obrigação legal ou regulatória** | Manutenção de prontuário conforme Resolução CFP nº 01/2009 |
| **Exercício regular de direitos** | Defesa em processos judiciais ou administrativos |
| **Proteção da vida ou incolumidade física** | Situações de risco ao paciente ou terceiros |
| **Tutela da saúde** | Prestação de serviços de saúde psicológica |

---

## 6. COMPARTILHAMENTO DE DADOS PESSOAIS

Os dados pessoais dos pacientes **NÃO serão vendidos, alugados ou compartilhados com terceiros**, exceto nas seguintes situações:

### 6.1. Com Consentimento do Paciente
O compartilhamento pode ocorrer quando houver **autorização expressa** do titular, como:
- Encaminhamento para outros profissionais de saúde (médicos, psiquiatras, etc.);
- Emissão de laudos, relatórios ou atestados solicitados pelo próprio paciente.

### 6.2. Por Obrigação Legal ou Regulatória
Os dados podem ser compartilhados para cumprimento de determinações legais, tais como:
- Atendimento a requisições judiciais ou do Conselho Federal de Psicologia (CFP);
- Notificação compulsória de doenças ou situações de risco à saúde pública;
- Defesa em processos administrativos ou judiciais.

### 6.3. Com Prestadores de Serviços Essenciais
Dados podem ser compartilhados com empresas contratadas para prestação de serviços necessários, sempre sob **acordos de confidencialidade**, incluindo:
- Serviços de armazenamento em nuvem seguro (Supabase/Lovable Cloud) com dados criptografados;
- Serviços de emissão de notas fiscais eletrônicas (FocusNFe ou similar);
- Contabilidade e gestão fiscal;
- Plataformas de infraestrutura de TI e segurança.

**Garantia:** Todos os prestadores de serviços estão obrigados a tratar os dados de acordo com a LGPD e com padrões rigorosos de segurança.

---

## 7. SEGURANÇA E PROTEÇÃO DOS DADOS

Adotamos medidas técnicas, administrativas e organizacionais para proteger os dados pessoais contra acessos não autorizados, perda, destruição, alteração ou uso indevido, incluindo:

🔒 **Criptografia de dados** em armazenamento e transmissão;  
🔒 **Controle de acesso restrito** aos dados, apenas por profissionais autorizados, com autenticação multifator (MFA);  
🔒 **Backup periódico e automático** das informações para prevenir perdas;  
🔒 **Logs de auditoria** para rastreamento de acessos e garantia de segurança;  
🔒 **Armazenamento de arquivos em buckets seguros** com controle de permissões por nível de acesso;  
🔒 **Atualizações de segurança** em sistemas e softwares utilizados;  
🔒 **Treinamento e conscientização** sobre boas práticas de proteção de dados.

**Observação:** Apesar de todas as medidas de segurança, nenhum sistema é 100% seguro. Em caso de incidente de segurança, o paciente será notificado conforme exigido pela LGPD.

---

## 8. PRAZO DE ARMAZENAMENTO DOS DADOS

Os dados pessoais serão armazenados pelo tempo necessário para as finalidades descritas nesta Política, observando:

- **Mínimo de 5 anos** após o término do atendimento, conforme Resolução CFP nº 01/2009;
- **Prazo superior**, quando houver obrigação legal de manutenção (ex: processos judiciais em andamento);
- **Exclusão ou anonimização** após o término do prazo legal, salvo autorização do titular para manutenção.

Após o prazo legal, os dados poderão ser:
- **Excluídos definitivamente**;
- **Anonimizados**, para uso em pesquisas ou estatísticas (sem identificação do paciente).

---

## 9. DIREITOS DOS TITULARES DE DADOS (PACIENTES)

Conforme a **LGPD (art. 18)**, os pacientes têm os seguintes direitos em relação aos seus dados pessoais:

| Direito | Descrição |
|---------|-----------|
| ✅ **Confirmação e acesso** | Solicitar confirmação sobre o tratamento dos dados e obter cópia deles |
| ✅ **Correção** | Solicitar a correção de dados incompletos, inexatos ou desatualizados |
| ✅ **Anonimização, bloqueio ou eliminação** | Pedir a anonimização ou exclusão de dados desnecessários ou tratados em desconformidade com a lei |
| ✅ **Portabilidade** | Solicitar a portabilidade dos dados para outro profissional ou serviço de saúde |
| ✅ **Eliminação de dados** | Solicitar a exclusão definitiva dos dados, exceto quando houver obrigação legal de manutenção |
| ✅ **Informação sobre compartilhamento** | Saber com quais entidades públicas ou privadas os dados foram compartilhados |
| ✅ **Revogação do consentimento** | Revogar o consentimento a qualquer momento, mediante comunicação expressa |
| ✅ **Oposição ao tratamento** | Opor-se ao tratamento dos dados em determinadas situações legais |

**Como exercer esses direitos?**  
Para exercer qualquer um desses direitos, o paciente deve entrar em contato com o(a) psicólogo(a) por meio dos canais informados no início desta Política.

**Prazo de resposta:** Até **15 dias** após a solicitação, conforme previsto na LGPD.

**Importante:** A revogação do consentimento pode inviabilizar a continuidade do tratamento psicológico, uma vez que o registro e armazenamento de informações clínicas são obrigatórios segundo a ética profissional.

---

## 10. TRANSFERÊNCIA INTERNACIONAL DE DADOS

**Os dados pessoais dos pacientes NÃO são transferidos para outros países**, exceto quando houver uso de serviços de nuvem com servidores no exterior. Nesses casos, garantimos que:
- A transferência está de acordo com a LGPD;
- Os prestadores de serviços possuem **certificações de segurança reconhecidas internacionalmente**;
- Há **cláusulas contratuais de proteção de dados**.

---

## 10.1. SISTEMA MULTIUSUÁRIO E HIERARQUIA DE ACESSO

Quando aplicável, o sistema pode operar em modelo multiusuário com diferentes níveis de acesso:

**Terapeutas:** Têm acesso completo aos dados dos seus próprios pacientes e sessões.

**Administradores:** Podem ter acesso aos dados de terapeutas subordinados e seus respectivos pacientes para fins de:
- Supervisão clínica e administrativa
- Gestão financeira e fiscal
- Auditoria e controle de qualidade
- Suporte operacional

**Garantias de segurança:**
- Todos os acessos são registrados em logs de auditoria
- Cada usuário possui credenciais individuais e intransferíveis
- Autenticação multifator (MFA) disponível para maior segurança
- Políticas de controle de acesso baseadas em função (RLS - Row Level Security)

**Importante:** Administradores só terão acesso aos dados quando houver relação de subordinação estabelecida, e todo acesso é registrado para fins de auditoria e compliance com a LGPD.

---

## 11. USO DE COOKIES E TECNOLOGIAS DE RASTREAMENTO

O sistema utiliza tecnologias modernas para gerenciamento de sessões e segurança, podendo coletar dados técnicos como:
- Informações de autenticação e sessão do usuário
- Endereço IP para fins de segurança e auditoria
- Informações sobre o dispositivo utilizado
- Dados de navegação para melhorar a experiência do usuário

**Armazenamento local:** O sistema pode utilizar armazenamento local do navegador para:
- Manter preferências do usuário
- Gerenciar sessões de autenticação
- Melhorar a performance da aplicação

**Controle:** O paciente pode limpar os dados armazenados através das configurações do navegador, porém isso pode afetar o funcionamento de algumas funcionalidades e exigir nova autenticação.

---

## 12. ALTERAÇÕES NESTA POLÍTICA DE PRIVACIDADE

Esta Política de Privacidade pode ser atualizada periodicamente para refletir mudanças na legislação, nas práticas de tratamento de dados ou nos serviços oferecidos.

Em caso de alterações significativas, os pacientes serão notificados por:
- E-mail;
- Mensagem durante o atendimento;
- Publicação em site (se aplicável).

**Data da última atualização:** [DATA]

---

## 13. CONTATO E DÚVIDAS

Para quaisquer dúvidas, solicitações ou reclamações sobre o tratamento de dados pessoais, entre em contato:

**Nome/Razão Social:** [NOME DO PSICÓLOGO OU CLÍNICA]  
**Encarregado de Dados (DPO):** [NOME DO RESPONSÁVEL]  
**Telefone:** [TELEFONE]  
**E-mail:** [E-MAIL]  
**Endereço:** [ENDEREÇO COMPLETO]

**Autoridade Nacional de Proteção de Dados (ANPD):**  
Em caso de violação de direitos, o titular pode registrar reclamação junto à ANPD:  
🔗 **Site:** [https://www.gov.br/anpd](https://www.gov.br/anpd)

---

## 14. DISPOSIÇÕES FINAIS

- Esta Política de Privacidade está em conformidade com a **Lei nº 13.709/2018 (LGPD)**, com o **Código de Ética Profissional do Psicólogo** e com as **Resoluções do Conselho Federal de Psicologia (CFP)**.
- O sigilo profissional é resguardado conforme **art. 9º do Código de Ética Profissional do Psicólogo**.
- Esta Política é parte integrante do **Termo de Consentimento para Tratamento de Dados Pessoais (LGPD)**.

---

**Ao continuar utilizando os serviços de psicoterapia oferecidos, o paciente declara ter lido, compreendido e aceito os termos desta Política de Privacidade.**

---

**Data:** _____ de ______________ de 20___  
**Responsável:** [NOME DO PSICÓLOGO]  
**CRP:** [NÚMERO]  
**Assinatura:** _________________________________
